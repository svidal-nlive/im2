"""
Spleeter Service for IM2 Audio Processing Pipeline

This service:
1. Separates audio files into stems using Spleeter
2. Supports different separation models (2stem, 4stem, 5stem)
...
class SpleeterModel(str, Enum):
    """Supported Spleeter separation models."""
    STEMS_2 = "2stems"  # Vocals + Accompaniment
    STEMS_4 = "4stems"  # Vocals + Drums + Bass + Other
    STEMS_5 = "5stems"  # Vocals + Drums + Bass + Piano + Other
...
class SeparationRequest(BaseModel):
    ...
    model: SpleeterModel = Field(default=SpleeterModel.STEMS_2, description="Separation model")
    bitrate: str = Field(default="256k", description="Output bitrate")
...
class SeparationResult(BaseModel):
    ...
    model: SpleeterModel = Field(..., description="Separation model")
...
class SpleeterService(BaseService):
    """Spleeter service for audio stem separation."""
    ...
    def __init__(self):
        ...
        super().__init__(
            name="spleeter",
            version="0.1.0",
            description="Audio separation service using Spleeter",
            dependencies=["queue", "splitter-stager"]
        )
        
        # Configure settings
        self.input_path = os.getenv("SPLEETER_INPUT_PATH", "/pipeline-data/spleeter-input")
        self.output_path = os.getenv("SPLEETER_OUTPUT_PATH", "/pipeline-data/spleeter-output")
        self.queue_service_url = os.getenv("QUEUE_SERVICE_URL", "http://queue:8003")
        self.polling_interval = int(os.getenv("POLLING_INTERVAL", "5"))
        self.default_model = SpleeterModel(os.getenv("DEFAULT_MODEL", SpleeterModel.STEMS_2))
    ...
    async def startup_event(self):
        ...
        logger.info("Spleeter service started")
    ...
    async def shutdown_event(self):
        ...
        logger.info("Spleeter service stopped")
    
    async def get_supported_models(self) -> List[str]:
        """Get list of supported separation models."""
        return [model.value for model in SpleeterModel]
    ...
    async def separate_file(self, request: Request) -> SeparationResult:
        ...
        try:
            ...
            return await self.separate_audio(
                ...
                bitrate=separation_request.bitrate
            )
    ...
    async def separate_audio(
        ...
        input_file: str,
        output_dir: str,
        model: str = SpleeterModel.STEMS_2,
        bitrate: str = "256k"
    ) -> SeparationResult:
        ...
        try:
            ...
            if isinstance(model, str):
                model = SpleeterModel(model)
            
            # Validate model and prepare stem configuration
            stem_names = []
            if model == SpleeterModel.STEMS_2:
                stem_names = ["vocals", "accompaniment"]
            elif model == SpleeterModel.STEMS_4:
                stem_names = ["vocals", "drums", "bass", "other"]
            elif model == SpleeterModel.STEMS_5:
            ...
            if proc.returncode != 0:
                logger.error(f"Spleeter command failed: {stderr.decode()}")
                raise RuntimeError(f"Spleeter command failed: {stderr.decode()}")
            ...
            for stem in stem_names:
                stem_file = os.path.join(output_subdir, f"{stem}.wav")
...
if __name__ == "__main__":
    ...
    port = int(os.getenv("SPLEETER_PORT", "8006"))
